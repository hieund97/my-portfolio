...
const Hero = ({ profile = {}, socialLinks = [] }) => {
  ...
  const socialIconsRendered = useMemo(() => 
    socialLinks.map((link) => {
      const IconComp = SOCIAL_ICONS[link.platform?.toLowerCase()];
  ...
  return (
    <section id="hero" className="hero-section relative min-h-screen flex items-center justify-center overflow-hidden py-20 lg:py-0 transition-colors duration-500">
      {/* Static background - no motion.div animation for glow */}
      <div className="absolute inset-0 -z-10 bg-slate-50 dark:bg-[#0B0F19]">
        {/* Grid Pattern via CSS only */}
        <div 
        ...
        {/* Static CSS glow effects instead of framer-motion animated ones */}
        <div className="hero-glow-1 absolute top-1/4 left-1/4 w-[600px] h-[600px] bg-primary-200/30 dark:bg-primary-900/30 rounded-full blur-[120px] mix-blend-multiply dark:mix-blend-screen pointer-events-none" />
        <div className="hero-glow-2 absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-cyan-200/20 dark:bg-cyan-900/20 rounded-full blur-[100px] mix-blend-multiply dark:mix-blend-screen pointer-events-none" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 lg:gap-8 items-center max-w-7xl mx-auto">
          ...
          >
            <div className="relative group perspective-1000 w-64 h-80 lg:w-full lg:max-w-[280px] lg:h-[400px]">
              <div className="absolute -inset-1 bg-gradient-to-br from-primary-500 via-purple-500 to-cyan-500 rounded-3xl blur opacity-20 dark:opacity-30 group-hover:opacity-40 dark:group-hover:opacity-60 transition duration-1000 group-hover:duration-200 hero-gradient-border"></div>
              ...
              >
                {/* Front Face */}
                <div 
                  className="absolute inset-0 w-full h-full rounded-3xl overflow-hidden hero-glass-panel border border-slate-200 dark:border-white/10 p-1 bg-white/5"
                ...
                {/* Back Face */}
                <div 
                  className="absolute inset-0 w-full h-full rounded-3xl overflow-hidden hero-glass-panel border border-slate-200 dark:border-white/20 p-1 bg-white/5"
          ...
          >
            <div className="space-y-4">
              ...
              <h1 className="text-5xl md:text-7xl font-display font-bold leading-tight text-slate-900 dark:text-white">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 via-purple-600 to-cyan-600 dark:from-primary-400 dark:via-purple-400 dark:to-cyan-400 hero-gradient-text">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 w-full">
                ...
                <div className="flex gap-4 w-full sm:w-auto">
                  <button 
                    ...
                    className="flex-1 sm:flex-none px-6 py-4 hero-glass-panel border border-slate-200 dark:border-white/10 text-slate-700 dark:text-white rounded-xl font-bold hover:bg-slate-100 dark:hover:bg-white/10 transition-all hover:scale-105 active:scale-95 flex items-center justify-center min-w-[140px]"
                  >
                    {t('hero.viewWork')}
                  </button>
